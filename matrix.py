...
import portal_bridge  # shared local cache the Flask portal (app.py) reads from
...
class ConfigManager:
    ...
    # Update APP_ID and API_TOKEN with your Deriv API keys
    DERIV_APP_ID: str = os.getenv("DERIV_APP_ID", "1089")  # 1089 is default testing App ID
    DERIV_API_TOKEN: str = os.getenv("DERIV_API_TOKEN", "YOUR_DERIV_API_TOKEN")
...
class TemporalSessionManager:
    ...
    def get_current_session(is_synthetic: bool = True) -> MarketSession:
        ...
        Synthetic indices operate 24/7/365 across all virtual sessions.
        """
        if is_synthetic:
...
class OrderFlowAnalyzer:
    ...
    def compute_volume_profile(df: pd.DataFrame, num_bins: int = 30) -> VolumeProfileNode:
        ...
        for _, row in df.iterrows():
            ...
            # Estimate Delta (Buy vs Sell volume bias based on close location)
...
class MultiTimeframeMatrix:
    ...
    async def sync_symbol(self, symbol: str) -> None:
        ...
        # Compute volume profile on M15 timeframe
...
class StrategyEvaluator:
    ...
    def evaluate_symbol(symbol: str, tf_data: Dict[str, pd.DataFrame], vp: Optional[VolumeProfileNode]) -> Optional[TradeSignal]:
        ...
        # Bullish Liquidity Sweep (Swept low, closed back above)
    ...
    async def _main_loop(self) -> None:
        ...
        while True:
            try:
                ...
                # 1. Update Account Equity & Risk State
                ...
                # 2. Synchronize Multi-Timeframe Data Matrix
                await self.matrix.sync_all_assets()

                # 3. Evaluate Trading Signals Across Assets
                ...
                # 4. Push live telemetry to the shared portal cache so the Flask
                #    dashboard (app.py) has something real to read. NOTE: Deriv's
                #    get_balance() doesn't separate balance from floating equity,
                #    so both fields carry the same figure for now — revisit if you
                #    add open-position P&L tracking later.
                portal_bridge.write_bot_telemetry(
                    balance=current_balance,
                    equity=current_balance,
                    regime="TRADING_HALTED" if self.risk_state.trading_halted else "SCANNING",
                ...
                # Maintain loop timing (~10-second sync cycles)
...
def start_bot() -> None:
    """
    Synchronous entry point for the bot engine.

    app.py imports this directly (`from matrix import start_bot`) and runs it on
    a background thread — this name has to exist and has to be a plain (non-async)
    callable for that to work, which is why it just wraps asyncio.run() rather than
    being a coroutine itself.
    """
    log.info("Starting Nexus Matrix Trading Engine (Deriv Cloud Edition)...")
...
if __name__ == "__main__":
    print("""
    =================================================================
       NEXUS MATRIX QUANTITATIVE ENGINE - DERIV CLOUD EDITION
    =================================================================
    """)
    start_bot()